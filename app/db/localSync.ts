...
export async function localSyncDB(options: {
  ...
  const localServer = await SyncServer.getByType("local")
  if (!localServer) {
    throw new Error("Local server not found")
  }

  // Get the last time we pulled from the local server
  const localServerLastPulledAt = (await SyncServer.getLastPullTimestamp("local")) || 0
  // const localServerLastPulledAt = 0

  // ask the local server for data that was updated or deleted since the last pull
  const res = await fetch(`${localServer.url}/sync?lastPulledAt=${localServerLastPulledAt}`, {
    method: "GET",
    headers,
  })
  ...
  try {
    ...
    await SyncServer.setLastPullTimestamp("local", serverTimestamp)
  ...
  try {
    ...
    const postRes = await fetch(`${localServer.url}/sync?lastPulledAt=${localServerLastPulledAt}`, {
      method: "POST",
      headers: postHeaders,
